...
;; --- Mail: mu4e + isync/mbsync + Migadu -------------------------------------
;; The mu indexer and mu4e (Emacs frontend) are installed via Nix.
;; Mail is synced locally with mbsync (isync).
;;
;; TODO: replace YOUR-EMAIL@YOUR-DOMAIN with your actual Migadu address.
;;       Then run:  M-x mu4e  to start.
;;
;; Credentials:
;;   echo -n 'your-password' > ~/.mbsyncpass && chmod 600 ~/.mbsyncpass
;;
;; First-time index (after syncing mail):
;;   mu init --maildir=~/.mail --my-address=YOUR-EMAIL@YOUR-DOMAIN
;;   mu index
;;
;; SMTP (sending):
;; Emacs uses smtpmail (built-in). The config below sends via
;; smtp.migadu.com:465 with STARTTLS, using the same credentials.
;; mu4e-compose-and-send-mail checks the password from auth-sources
;; (see ~/.authinfo.gpg or the plstore approach).
;;
;; Key bindings (mu4e headers/view modes):
;;   RET / mouse-1  open message
;;   j               jump to maildir
;;   C-c C-u         refresh (sync + reindex)
;;   q               quit
;;   U               mark for refiling (archive)
;;   x               execute marks

;; Guarded: config.el is a live checkout and must stay loadable even before
;; the Nix rebuild that adds mu4e lands. If mu4e is not available the block
;; is skipped cleanly.
(when (require 'mu4e nil :no-error)
  ;; Maildir path — matches ~/.mbsyncrc
  (setq mu4e-maildir (expand-file-name "~/.mail/migadu"))

  ;; Default mu4e directory structure — no fancy separation, just standard
  ;; Maildir++ folders created by mbsync + Migadu's auto-creation.
  (setq mu4e-root-maildir "/migadu")

  ;; Sync command (mu index runs automatically after mu4e startup)
  (setq mu4e-get-mail-command "mbsync -a")
  ;; Run this on `mu4e-update-mail' (C-c C-u in mu4e headers)
  (setq mu4e-update-interval nil) ; manual sync only

  ;; My own email address — used for "To me" filtering, identity etc.
  (setq mu4e-user-mail-address-list '("YOUR-EMAIL@YOUR-DOMAIN"))

  ;; Don't confirm before sending
  (setq mu4e-compose-dont-reply-to-self t
        mu4e-confirm-quit nil
        mu4e-headers-leave-behavior 'all
        mu4e-view-show-images t
        mu4e-view-show-addresses t)

  ;; --- Sending mail (SMTP) ---
  ;; Emacs built-in smtpmail, configured for smtp.migadu.com:465
  ;;
  ;; Credentials are stored in ~/.authinfo.gpg (Emacs netrc format).
  ;; Create it with:
  ;;   machine smtp.migadu.com login YOUR-EMAIL@YOUR-DOMAIN password your-password port 465
  ;;
  ;; Or use the plstore approach (safer):
  ;;   M-x auth-source-save  (creates ~/.authinfo.gpg)
  ...
  ;; mu4e context: single Migadu account
  ;; If you add more accounts later, add more contexts.
  (setq mu4e-contexts
        `(,(make-mu4e-context
            :name "Migadu"
            :enter-func (lambda () (mu4e-message "Entering Migadu context"))
            :leave-func (lambda () (mu4e-message "Leaving Migadu context"))
            :match-func (lambda (msg)
                          (when msg
                            (mu4e-message-contact-field-matches
                             msg '(:to :cc :bcc) "YOUR-EMAIL@YOUR-DOMAIN")))
            :vars '((user-mail-address . "YOUR-EMAIL@YOUR-DOMAIN")
                    (user-full-name . "Scott Whitson") ;; ← TODO: update your name
  ...
  ;; Default context (used when no other matches)
  (setq mu4e-context-policy 'pick-first
        mu4e-compose-context-policy 'always)

  ;; mu4e main view with one account
  (setq mu4e-bookmarks
        `(( ,(make-mu4e-bookmark
              :name "Inbox"
              :query "maildir:/migadu/INBOX AND flag:new"
              :key ?i)
           ,(make-mu4e-bookmark
             :name "All unread"
  ...
  ;; Optional: show a shortcut to mu4e on C-c m
