;;; eminix-prose-test.el --- ERT tests -*- lexical-binding: t; -*-
(require 'ert)
(require 'markdown-mode)
(require 'org)
(require 'eminix-prose)

(ert-deftest eminix-prose-palette-color-known-key ()
  "A key present in the active theme's palette returns a hex colour."
  (let ((c (eminix/theme-palette-color "surface0")))
    (should (or (null c) (string-match-p "\\`#[0-9a-fA-F]\\{6\\}\\'" c)))))

(ert-deftest eminix-prose-palette-color-unknown-key ()
  "An absent key returns nil rather than signalling."
  (should (null (eminix/theme-palette-color "no-such-palette-key"))))

(ert-deftest eminix-prose-remaps-cover-markdown-faces ()
  "Every markdown face the design names is in the remap table."
  (with-temp-buffer
    (markdown-mode)
    (let ((faces (mapcar #'car (eminix-prose--face-remaps))))
...
(ert-deftest eminix-prose-tables-and-code-stay-monospace ()
  "Table and code faces are remapped to the mono family, not the body font."
  (with-temp-buffer
    (markdown-mode)
    (let ((remaps (eminix-prose--face-remaps)))
      (dolist (f '(markdown-table-face markdown-code-face
        ...
        (should (equal (plist-get (alist-get f remaps) :family)
                       eminix-prose-mono-font))))))

(ert-deftest eminix-prose-headings-use-the-heading-font-and-descend ()
  ...
  (with-temp-buffer
    ...
    (let* ((remaps (eminix-prose--face-remaps))
      ...
      (should (equal (plist-get (alist-get 'markdown-header-face-1 remaps) :family)
                     eminix-prose-heading-font))
      (should (equal scales (sort (copy-sequence scales) #'>)))
      (should (> (car scales) 1.0)))))

(ert-deftest eminix-prose-enabling-adds-cookies-disabling-removes-them ()
  "The mode leaves no face-remap state behind when switched off."
  (with-temp-buffer
    (markdown-mode)
    (should (null eminix-prose--cookies))
    (eminix-prose-mode 1)
    (should (> (length eminix-prose--cookies) 0))
    (eminix-prose-mode -1)
    (should (null eminix-prose--cookies))))

(ert-deftest eminix-prose-toggling-is-idempotent ()
  "Enabling twice then disabling twice ends in a clean buffer."
  (with-temp-buffer
    (markdown-mode)
    (eminix-prose-mode 1)
    (let ((n (length eminix-prose--cookies)))
      (eminix-prose-mode 1)
      (should (= n (length eminix-prose--cookies))))
    (eminix-prose-mode -1)
    (eminix-prose-mode -1)
    (should (null eminix-prose--cookies))))

(ert-deftest eminix-prose-restores-line-numbers-on-disable ()
...
(ert-deftest eminix-prose-leaves-line-numbers-off-if-they-were-off ()
...
(ert-deftest eminix-prose-double-enable-does-not-forget-line-numbers ()
...
(defmacro eminix-prose-test--with-md (text &rest body)
...
(defun eminix-prose-test--invisible-count ()
...
(ert-deftest eminix-prose-hides-markup ()
  "With the mode on, emphasis markers carry an invisible property."
  (eminix-prose-test--with-md "Some **bold** text\n\nMore text\n"
    (should (> (eminix-prose-test--invisible-count) 0))))

(ert-deftest eminix-prose-reveals-the-line-at-point ()
  "Markup on the line at point is revealed; markup elsewhere is not."
  (eminix-prose-test--with-md "Some **bold** text\n\nAnd `code` here\n"
    (goto-char (point-min))
    (eminix-prose--reveal-at-point)
    (should (equal eminix-prose--revealed
...
(ert-deftest eminix-prose-rehides-when-point-leaves ()
  "Moving to another line restores the markup that was revealed."
  (eminix-prose-test--with-md "Some **bold** text\n\nAnd `code` here\n"
    (goto-char (point-min))
    (eminix-prose--reveal-at-point)
    (let ((revealed (eminix-prose-test--invisible-count)))
      (goto-char (point-max))
      (eminix-prose--reveal-at-point)
      (should (> (eminix-prose-test--invisible-count) revealed)))))

(ert-deftest eminix-prose-disabling-clears-reveal-state ()
  "Turning the mode off drops the reveal hook and its state."
  (eminix-prose-test--with-md "Some **bold** text\n"
    (eminix-prose--reveal-at-point)
    (should eminix-prose--revealed)
    (eminix-prose-mode -1)
    (should (null eminix-prose--revealed))
    ...
    (should (not (memq #'eminix-prose--reveal-at-point post-command-hook)))))

(ert-deftest eminix-prose-detects-image-links ()
  "Image detection is true only for buffers containing an image link."
  (with-temp-buffer
    (insert "A [link](http://x) but no picture\n")
    (markdown-mode)
    (should (null (eminix-prose--buffer-has-images-p))))
  (with-temp-buffer
    (insert "Here: ![alt](assets/diagram.png)\n")
    (markdown-mode)
    (should (eminix-prose--buffer-has-images-p))))

(ert-deftest eminix-prose-sets-up-the-reading-column ()
  "Enabling the mode establishes the centered column and visual wrapping."
  (eminix-prose-test--with-md "Body text\n"
    (should (bound-and-true-p visual-line-mode))
    (should (= visual-fill-column-width eminix-prose-width))
    (should visual-fill-column-center-text)))

(ert-deftest eminix-prose-displays-list-bullets ()
  "An unordered list marker gets a bullet display property."
  (eminix-prose-test--with-md "- first item\n- second item\n"
    (goto-char (point-min))
    (should (equal (get-text-property (point) 'display) "•"))))

(ert-deftest eminix-prose-leaves-ordered-lists-alone ()
  "Numbered list markers are not replaced."
  (eminix-prose-test--with-md "1. first item\n"
    (goto-char (point-min))
    (should (null (get-text-property (point) 'display)))))

(ert-deftest eminix-prose-removes-bullets-on-disable ()
...
(ert-deftest eminix-prose-leaves-hyphen-thematic-breaks-alone ()
...
so the assertion is that it is not OUR bullet, not that it is absent."
  (eminix-prose-test--with-md "Text\n\n- - -\n\nMore\n"
    (goto-char (point-min))
    (search-forward "- - -")
    (beginning-of-line)
    (should (not (equal (get-text-property (point) 'display) "•")))))

(ert-deftest eminix-prose-leaves-asterisk-thematic-breaks-alone ()
  "A `* * *' horizontal rule does not get a bullet stamped on it.
markdown-mode's own `markdown-hide-markup' feature (on since Task 3)
legitimately puts its own `display' property here — a rendered hr line —
so the assertion is that it is not OUR bullet, not that it is absent."
...
(ert-deftest eminix-prose-leaves-major-mode-managed-props-intact ()
...
(ert-deftest eminix-prose-remaps-cover-org-faces ()
...
(ert-deftest eminix-prose-org-tables-stay-monospace ()
  "org-table is remapped to the mono family."
  (with-temp-buffer
    (org-mode)
    (should (equal (plist-get (alist-get 'org-table (eminix-prose--face-remaps))
                              :family)
                   eminix-prose-mono-font))))

(ert-deftest eminix-prose-org-hides-emphasis-markers ()
...
(ert-deftest eminix-prose-org-does-not-install-the-markdown-reveal-hook ()
...
(ert-deftest eminix-prose-org-enables-org-modern-and-org-appear ()
...
(ert-deftest eminix-prose-org-disables-org-modern-and-org-appear ()
...
(provide 'eminix-prose-test)
;;; eminix-prose-test.el ends here
