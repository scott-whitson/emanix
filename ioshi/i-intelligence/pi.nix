...
in
{
  ...
  config = {
    home = {
      ...
      activation.piSettingsSeed = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      ...
      file = {
        ...
        ".pi/agent/auth.json" = lib.mkIf config.eminix.pi.enable {
        ...
        # Pi themes are generated by lib/gen-pi-theme.py at build time, inside
        # the theme-tree derivation (lib/theme-tree.nix), not during bootstrap.
        # HM doesn't manage the generated output — the runtime theme switcher
        # just points settings.json at the already-built file.
