...
Each theme is a self-contained directory, generated by Emanix itself —
...
in a checkout. That path is exposed as the option `emanix.src.themesDir` and,
for shell scripts, the environment variable `EMANIX_THEMES_DIR`; consumers may
...
by `lib/gen-pi-theme.py` (a script Emanix carries), reading the same
...
override of `emanix.src.themesDir`) to start supplying the file, and the
...
top bar) is themed by calling `emanix/theme-set` in the running daemon; pi's
...
rendered by Nix from `config.emanix.theme` at build time. That is **not** the
...
ever; only editing `emanix.theme` in `host config` and rebuilding
...
5. Symlinks `available/emanix-<variant>.kdl` → `active/theme.kdl` in
   `~/.local/share/dotfiles/zellij-themes/`. Both definitions are named
   `emanix`, so zellij's `theme` line never changes.
...
9. Calls `(emanix/theme-set "<name>")` in the running Emacs daemon, resolving
   `emacsclient` from `PATH`. Emacs maps the name via
   `$EMANIX_THEMES_DIR/<name>/emacs-theme`.
...
`emanix-dark.kdl` and `emanix-light.kdl` use zellij's verbose per-declaration
...
themes by listing `$EMANIX_THEMES_DIR`, and Emacs resolves the theme through
`$EMANIX_THEMES_DIR/<name>/emacs-theme`.
...
`$EMANIX_THEMES_DIR/*/helix-theme` files and the sed block itself. That
...
(`$EMANIX_THEMES_DIR`) built once at rebuild time, so `dot-theme-set` has
...
`post-set.sh`, or a per-host override of `emanix.src.themesDir` pointing at a
