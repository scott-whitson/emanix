...
| 07 | `07-pi.sh` | Ensures node/npm are present, installs the pi coding agent via `npm install -g --prefix ~/.local @earendil-works/pi-coding-agent`, and uses the stowed `~/.local/bin/pi` launcher with a fallback to `npx` if the local install is missing or broken. | None |
