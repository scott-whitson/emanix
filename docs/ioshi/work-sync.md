...
versioned hub) to rafik. Spec:
`docs/superpowers/specs/2026-07-19-three-node-home-model-design.md`.

| Share | WSL path | datacore/rafik path | Devices |
| --- | --- | --- | --- |
| `work-projects` | `~/projects` (all of it) | `~/projects/work` | WSL ↔ datacore ↔ rafik |
| `work-docs` | `~/docs/org/work` | `~/docs/org/work` | WSL ↔ datacore (rafik gets it via `docs`) |
...
- Single-writer discipline: the WSL is the writing machine for work repos;
  treat datacore/rafik copies as read-mostly. Recovery = datacore's
...
- WSL syncthing is HM-managed (`ioshi/i-intelligence/syncthing.nix`);
  datacore is Debian-managed (REST); rafik is the NixOS module.
