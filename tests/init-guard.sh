...
#
# NOT wired as a flake check: it builds a temp tree and runs Emacs in batch
# mode (real subprocess I/O, timeouts, a live filesystem), which is a heavier
# and more fragile thing to do at Nix eval/build time inside the sandbox than
# palette-contrast's pure-function check is. Run it by hand instead:
#   ./tests/init-guard.sh
#
